...
def run_niche_search(config, niches=None, rules=None, discover_fn=None):
    ...
    if discover_fn is not None:
        raw, raw_count = discover_fn(queries, locations, config)
    elif source == "playwright":
        raw, raw_count = discover_playwright(queries, locations, config, history)
    else:
        if not GOOGLE_API_KEY or GOOGLE_API_KEY == "YOUR_GOOGLE_API_KEY":
            raise RuntimeError("Please set GOOGLE_API_KEY in .env before running API Manager niche search.")
        raw, raw_count = discover_api_manager(
            queries, locations, config, GOOGLE_API_KEY, api_call_counter, history
        )
    ...
    if config.max_leads:
        qualified = qualified[: int(config.max_leads)]

    high_intent = sum(1 for lead in qualified if int(lead.get("lead_score") or 0) >= 70)
    score_55 = sum(1 for lead in qualified if int(lead.get("lead_score") or 0) >= 55)
    stats = {
        "niche": niche["display_name"],
        "locations": [f"{city}, {state}" if state else str(city) for state, city, _ in locations],
    ...
    log_stage(6, "Output", f"{len(qualified)} qualified leads")
    if config.output_mode in ("json", "both"):
        save_results(qualified, config.json_output)
    if config.output_mode in ("dashboard", "both"):
        send_to_dashboard(qualified)
    history.record_run({
        "started_at": _now_iso(),
        "leadgen_type": source,
        "niche": niche["id"],
